...
<div class="col-md-3 pull-left" id="userWidget">
	<div class="panel widget" style="box-shadow: 1px 1px 1px 1px rgba(0,0,0,.05);">
		...
		<div class="panel-body text-center">
			<a href="<?php echo $domain?>/app/profile.php?id=<?php echo $requester['id']?>" class="linkblk">
				<img src="<?php echo getProfilePicture($domain,$requester)?>" alt="<?php echo $requester['full_name']?>" class="center-block img-thumbnail img-circle thumb96"> <br>
				<p style="font-size:17px;"><b> <?php echo $split_name['first_name']?> </b></p>
			</a>
			<a href="?accept=true&id=<?php echo $requester['id']?>" class="text-success pull-left"> <i class="fa fa-check fa-fw fa-lg"></i> <?php echo $lang['Accept']?> </a> <a href="?decline=true&id=<?php echo $requester['id']?>" class="text-danger pull-right"> <i class="fa fa-times fa-fw fa-lg"></i> <?php echo $lang['Decline']?> </a> 
		</div>
	</div>
</div>
	...
	while($friend = $friends->fetch_array()) { 
		...
		<div class="col-md-3 pull-left" id="userWidget">
			<div class="panel widget" style="box-shadow: 1px 1px 1px 1px rgba(0,0,0,.05);">
				...
				<div class="panel-body text-center">
					<a href="<?php echo $domain?>/app/profile.php?id=<?php echo $fr['id']?>" class="linkblk">
			...
			<ul class="pagination pagination-sm m0 pull-left">
				...
				if($last_page < $p) {
					for($i=1; $i<=$last_page; $i++) {
						if($i == $p) {
							?>
							<li class="active"> <a href="<?php echo $domain?>/app/my-friends.php?page=<?php echo $i?>"> <?php echo $i; ?> </a> </li>
							<?php
						} else {
							?>
							<li> <a href="<?php echo $domain?>/app/my-friends.php?page=<?php echo $i?>"> <?php echo $i; ?> </a> </li>
							<?php	
						}
					}
				}
